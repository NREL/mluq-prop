...
def get_scaler_inpt(scaler):
    if scaler is None:
        with open(os.path.join(DATA_DIR, "scalerInput_upgraded.pkl"), "rb") as handle:
...
def testInpt():
    scalerInpt_prop = np.load(os.path.join(DATA_DIR,"scaling_input_prop.npz"))
...
def testOtpt():
    scalerOtpt_prop = np.load(os.path.join(DATA_DIR, "scaling_output_prop.npz"))
