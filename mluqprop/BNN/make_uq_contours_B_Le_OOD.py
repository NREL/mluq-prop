...
maxC = 0.2512863355
diff = 8.249726452762996e-05
chi_lam = 70.2452780212977
rhoOmega = 599.68144

data_fw4 = np.load(os.path.join(DATA_DIR, "B_Le_fw4.npz"))
data_fw16 = np.load(os.path.join(DATA_DIR, "B_Le_fw16.npz"))
...
#inpt_fw4_scaled_old = np.load("orig_scaler/B_Le_fw4_input_scaleold.npy")
#inpt_fw16_scaled_old = np.load("orig_scaler/B_Le_fw16_input_scaleold.npy")
#otpt_fw4_scaled_old = np.load("orig_scaler/B_Le_fw4_output_scaleold.npy")
#otpt_fw16_scaled_old = np.load("orig_scaler/B_Le_fw16_output_scaleold.npy")
...
for i in range(10):
    ...
    print(f"\tB_LE FW4 min {min_ble_4:.2f} max {max_ble_4:.2f}")
    print(f"\tB_LE FW16 min {min_ble_16:.2f} max {max_ble_16:.2f}")
print(f"output")
...
print(f"\tB_LE UNSCALED FW4 min {min_ble_4:.2f} max {max_ble_4:.2f}")
print(f"\tB_LE UNSCALED FW16 min {min_ble_16:.2f} max {max_ble_16:.2f}")


def find_OOD(pred, aleat, mean_tar = 0, std_tar=2.1047089292886376):
    mean_pred = np.mean(pred, axis=0, keepdims=True)
    mean_aleat = np.mean(aleat, axis=0, keepdims=True)
    dist_m = np.linalg.norm(mean_pred-mean_tar, axis=0)
    dist_s = np.linalg.norm(mean_aleat-std_tar, axis=0)
    dist = dist_m + dist_s
    return dist, dist_m, dist_s
...
dist_fw4, dist_m_fw4, dist_s_fw4 = find_OOD(predicted_fw4, aleatory_fw4)
dist_fw16, dist_m_fw16, dist_s_fw16 = find_OOD(predicted_fw16, aleatory_fw16)
indOOD_fw4 = np.argwhere(dist_fw4<1.5)
indOOD_fw16 = np.argwhere(dist_fw16<1.5)
...
in_dist_fw4 = np.ones(epipreds_mean_fw4.shape)
in_dist_fw16 = np.ones(epipreds_mean_fw16.shape)

in_dist_fw4[indOOD_fw4]=0
in_dist_fw16[indOOD_fw16]=0

pred_fw4 = np.reshape(epipreds_mean_fw4, (x_len, y_len))
pred_fw16 = np.reshape(epipreds_mean_fw16, (x_len, y_len))
in_dist_fw4 = np.reshape(in_dist_fw4, (x_len, y_len))
in_dist_fw16 = np.reshape(in_dist_fw16, (x_len, y_len))
#plot_2d(pred_fw4)
#plot_2d(pred_fw16)
...
os.makedirs(os.path.join("Figures", "B_Le10D"), exist_ok = True)
figureFolder = os.path.join("Figures", "B_Le10D")


pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
                     [in_dist_fw4, in_dist_fw16],
...
plt.savefig(os.path.join(figureFolder, "chiSFS_B_Le_fw4.png"))
...
plt.savefig(os.path.join(figureFolder, "chiSFS_B_Le_fw16.png"))

pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
                     [np.clip(np.reshape(otpt_fw4, (x_len, y_len)) + np.reshape(inpt_fw4[:,2],(x_len, y_len))*chi_lam, a_min=0, a_max=None)/chi_lam , np.clip(pred_fw4 * chi_lam + np.reshape(inpt_fw4[:,2]*chi_lam,(x_len, y_len)), a_min=0.01, a_max=None)/chi_lam],
                     xbound=[x[x_min], x[x_max-1]],
                     ybound=[y[0], y[-1]],
                     listTitle=["True", "BNN"],
...
plt.savefig(os.path.join(figureFolder, "chiF_B_Le_fw4.png"))
pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
                     [np.clip(np.reshape(otpt_fw16, (x_len, y_len)) + np.reshape(inpt_fw16[:,2],(x_len, y_len))*chi_lam, a_min=0, a_max=None)/chi_lam, np.clip(pred_fw16 * chi_lam + np.reshape(inpt_fw16[:,2],(x_len, y_len))*chi_lam, a_min=0.01, a_max=None)/chi_lam],
                     xbound=[x[x_min], x[x_max-1]],
                     ybound=[y[0], y[-1]],
                     listTitle=["True", "BNN"],
                     listCBLabel=[r"$\widetilde{\chi}$ / $\chi_{\rm lam} [-]$",  r"$\widetilde{\chi}$ / $\chi_{\rm lam} [-]$"],
...
plt.subplots_adjust(wspace=0.4)
plt.savefig(os.path.join(figureFolder, "chiF_B_Le_fw16.png"))
pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
...
plt.subplots_adjust(wspace=0.4)
plt.savefig(os.path.join(figureFolder, "unc_B_Le_fw4.png"))
pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
...
plt.subplots_adjust(wspace=0.4)
plt.savefig(os.path.join(figureFolder, "unc_B_Le_fw16.png"))
pretty_multi_contour(
                     [x[x_min:x_max]],
...
plt.savefig(os.path.join(figureFolder, "ratio_unc_B_Le_fw4.png"))
pretty_multi_contour(
                     [x[x_min:x_max]],
...
plt.savefig(os.path.join(figureFolder, "ratio_unc_B_Le_fw16.png"))
pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
                     [np.reshape(epistd_fw4, (x_len, y_len)), np.reshape(alestd_fw4,(x_len, y_len))],
                     xbound=[x[x_min], x[x_max-1]],
...
plt.subplots_adjust(wspace=0.4)
plt.savefig(os.path.join(figureFolder, "logunc_B_Le_fw4.png"))
pretty_multi_contour(
                     [x[x_min:x_max], x[x_min:x_max]],
                     [np.reshape(epistd_fw16, (x_len, y_len)), np.reshape(alestd_fw16,(x_len, y_len))],
                     xbound=[x[x_min], x[x_max-1]],
...
plt.subplots_adjust(wspace=0.4)
plt.savefig(os.path.join(figureFolder, "logunc_B_Le_fw16.png"))
plt.show()
